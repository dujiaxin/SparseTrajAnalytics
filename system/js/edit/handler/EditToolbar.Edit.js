	...
	save: function () {
		...
		this._featureGroup.eachLayer(function (layer) {
			if (layer.edited) {
				...
				layer.edited = false;
				//console.log(layer.getLatLngs());        //Shamal
				map.removeLayer(drawnItems2[layer.id]);
				//drawnItems2 = new L.FeatureGroup();
				//map.addLayer(drawnItems2);
				//Prepare rectangle border for query
				var coor = [];
				if((layer.type === 'rectangle')||(layer.type === 'polygon')){
				...
				var T1 = "2013-09-02 07:00:00";
				var T2 = "2013-09-02 09:00:00";
				var SMM;
				if (SM == "Intersect") {
					SMM = "1";
				} else if (SM == "Pick-UP") {
				...
				doquery(stcor, layer.id, layer, layer.type, SM);
				}
				else
					if (layer.type === 'circle') {
						var r = new L.LayerGroup();
						var desyCircle = LGeo.circle([layer.getLatLng().lat, layer.getLatLng().lng], layer.getRadius(), {parts: 144}).addTo(r);
						var f = r.getLayers()
						var p = f[0].toGeoJSON()
