...
if(count($input1) > 4 && (int)$input1[5] == 1)
	{
		 ...
		 $SQL="with x as (select geom from regions where ST_Intersects(ST_GeomFromText('POLYGON((". $input1[0]."))',4326), geom)) SELECT DISTINCT tripid,ST_AsText(trip) as trajectorypoints,St_Length(trip::geography) as len FROM (SELECT orids,
           generate_subscripts(orids, 1) AS s,tripid,starttime,trip
      FROM tdr) AS foo WHERE orids[s] in  " .$input1[4]. ",x WHERE starttime BETWEEN '".$input1[1]."' AND '".$input1[2]."' AND "; 
...
//group by Months=========================
  $qrMonth = "SELECT startMonth,array_agg(tripid),count(*) FROM (Select  tripid,EXTRACT(Month FROM min(starttime)) as startMonth from tdr WHERE tripid in" . $st2 . " group by tripid ) as x group by startMonth order by startMonth";
  ...
  //group by Years=========================
  $qrYear = "SELECT startYear,array_agg(tripid),count(*) FROM (Select  tripid,EXTRACT(Year FROM min(starttime)) as startYear from tdr WHERE tripid in" . $st2 . " group by tripid ) as x group by startYear order by startYear";
  ...
  $qr6="select regionid,count(*) as total,avg(speed)as avspeed,MAX(speed) as maxspeeed,MIN(speed) as minspeed from(SELECT unnest(orids) as regionid,unnest(speeds)as speed FROM tdr WHERE tripid in".$st2.") as f group by regionid order by count(*) DESC";
