...
function GetTripsIntersect($input, $conn) {
    ...
    if ((int)$input1[3] == 4) {
        ...
        $qr6 = "select roadid,count(*) as total,avg(speed)as avspeed from(Select  rid as roadid,speed from gps_points_rn where pdatetime BETWEEN '" . $input1[1] . "' AND '" . $input1[2] . "' And ST_Intersects(ST_GeomFromText('POLYGON((" . $input1[0] . "))',4326), point)) as f group by roadid order by 2 DESC";
        ...
        $qr6 = "select roadid,count(*) as total,avg(speed)as avspeed from(SELECT unnest(orids) as roadid,unnest(speeds)as speed FROM tds WHERE tripid in" . $st2 . ") as f group by roadid order by count(*) DESC";
        $query6 = $conn->prepare($qr6);
        $query6->execute();
        $record6 = $query6->fetchAll();
        $CStreet_Rank = "";
