...
function NGetTripsIntersect($input, $conn) {
	...
	if(count($input1) > 4 && (int)$input1[5] == 1)
	{
       ...
       $Trip_Id_List = "(";
        ...
        //group by Months=========================
        $qrMonth = "SELECT startMonth,array_agg(tripid),count(*) FROM (Select  tripid,EXTRACT(Month FROM min(PDateTime)) as startMonth from gps_points1 where pdatetime BETWEEN '" . $input1[1] . "' AND '" . $input1[2] . "' AND tripid in'" .$input1[4]. "' AND ST_Intersects(ST_GeomFromText('POLYGON((" . $input1[0] . "))',4Month26), point) group by tripid ) as x group by startMonth order by startMonth";
        ...
        //===============================================================================
        //group by Year=========================
        $qrYear = "SELECT startYear,array_agg(tripid),count(*) FROM (Select  tripid,EXTRACT(Year FROM min(PDateTime)) as startYear from gps_points1 where pdatetime BETWEEN '" . $input1[1] . "' AND '" . $input1[2] . "' AND tripid in'" .$input1[4]. "' AND ST_Intersects(ST_GeomFromText('POLYGON((" . $input1[0] . "))',Year326), point) group by tripid ) as x group by startYear order by startYear";
	  ...
	  elseif ((int)$input1[3]==3) {
        ...
        //group by Months=========================
        $qrMonth = "SELECT startMonth,array_agg(tripid),count(*) FROM td WHERE tripid in" . $st2 . "group by startMonth order by startMonth";
        $queryMonth = $conn->prepare($qrMonth);
        $queryMonth->execute();
        $recordMonth = $queryMonth->fetchAll();
        $Months_Array = array();
        ...
        //===============================================================================
        //group by Year=========================
        $qrYear = "SELECT startYear,array_agg(tripid),count(*) FROM td  WHERE tripid in" . $st2 . "group by startYear order by startYear";
        $queryYear = $conn->prepare($qrYear);
        $queryYear->execute();
        $recordYear = $queryYear->fetchAll();
        $Years_Array = array();
    ...
    if ((int)$input1[3] == 4) {
       ...
       $Trip_Id_List = "(";
        ...
        //group by Months=========================
        $qrMonth = "SELECT startMonth,array_agg(tripid),count(*) FROM (Select  tripid,EXTRACT(Month FROM min(PDateTime)) as startMonth from gps_points1 where pdatetime BETWEEN '" . $input1[1] . "' AND '" . $input1[2] . "' And ST_Intersects(ST_GeomFromText('POLYGON((" . $input1[0] . "))',4Month26), point) group by tripid ) as x group by startMonth order by startMonth";
        $queryMonth = $conn->prepare($qrMonth);
        $queryMonth->execute();
        $recordMonth = $queryMonth->fetchAll();
        $Months_Array = array();
        ...
        //===============================================================================
        //group by Year=========================
        $qrYear = "SELECT startYear,array_agg(tripid),count(*) FROM (Select  tripid,EXTRACT(Year FROM min(PDateTime)) as startYear from gps_points1 where pdatetime BETWEEN '" . $input1[1] . "' AND '" . $input1[2] . "' And ST_Intersects(ST_GeomFromText('POLYGON((" . $input1[0] . "))',Year326), point) group by tripid ) as x group by startYear order by startYear";
        $queryYear = $conn->prepare($qrYear);
        $queryYear->execute();
        $recordYear = $queryYear->fetchAll();
        $Years_Array = array();
