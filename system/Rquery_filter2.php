...
// rank trips by trip length===============
 $SQL="SELECT DISTINCT tripid,ST_AsText(trip) as trajectorypoints,St_Length(trip::geography) as len FROM tdr "
  ...
  $SQL2="select regionid,count(*) as total,avg(speed)as avspeed,MAX(speed) as maxspeed,MIN(speed) as minspeed from(SELECT regionid,speed from(SELECT unnest(orids) as regionid,
